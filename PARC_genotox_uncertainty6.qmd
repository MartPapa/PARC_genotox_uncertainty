...
EFSAgenotoxDB_filtered <- EFSAgenotoxDB %>%
 ...
 filter(!(sub_name != com_name & apply(select(., 7:46), 1, function(x) length(unique(x)) == 1)))%>%
  select(-sub_name, -sub_ecsubinvententryref, -sub_casnumber, -sub_type, -qualifier, -iupacname, -comparamname,-molecularformula,-smilesnotation, -inchi,-smilesnotationsource, -inchi_notationsource,-dar_author, -dar_title, -dar_year, -opinion_title, -publicationdate, -doi, -opinion_type, -opinion_author,-com_structureshown, -com_type, -testtype, -genotox_endpoint, -com_ecsubinvententryref)

write_xlsx(EFSAgenotoxDB_filtered, here::here("tables", "EFSAgenotoxDB_filtered.xlsx"))

EFSAgenotoxDB_filtered_min3_replicates <- EFSAgenotoxDB_filtered %>%
  group_by(genotoxguidelinefulltxt, com_name)%>%
  mutate(n_repl_total= n())%>%
  filter(n_repl_total >2)

write_xlsx(EFSAgenotoxDB_filtered_min3_replicates, here::here("tables", "EFSAgenotoxDB_filtered_min3_replicates.xlsx"))

EFSAgenotoxDB_filtered_min3_literature_studies <- EFSAgenotoxDB_filtered %>%
  group_by(genotoxguidelinefulltxt, com_name, literature_reference)%>%
  mutate(n_repl_lit= n())%>%
  filter(n_repl_lit >2)

write_xlsx(EFSAgenotoxDB_filtered_min3_literature_studies, here::here("tables", "EFSAgenotoxDB_filtered_min3_literature_studies.xlsx"))
...
# Create the plot with the new sequence of the labels
plot <- ggplot(long_data_joined, aes(x = TG_analysis_label, y = Percentage, fill = Category)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(
    values = c("perc_above85" = "green", "perc_66_85" = "yellow", "perc_below66" = "red"),
    labels = c("perc_above85" = "above85", "perc_66_85" = "66-85", "perc_below66" = "below66")
  ) +
  theme_minimal() +
  labs(x = "", y = "Percentage", fill = "Range Category") +
  ggtitle("") +
