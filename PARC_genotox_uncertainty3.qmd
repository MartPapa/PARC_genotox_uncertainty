...
EFSAgenotoxDB_inVivo_probs <- EFSAgenotoxDB_inVivo %>%
  select(!opinion_type)%>%
...
EFSAgenotoxDB_inVivo_probs_sum_474 <- EFSAgenotoxDB_inVivo_probs%>%
...
summarise(
  ...
  mutate(TG_analysis = "TG474_UnGrouped")

EFSAgenotoxDB_inVivo_probs_sum_475 <- EFSAgenotoxDB_inVivo_probs%>%
  filter(str_detect(genotoxguidelinefulltxt,"475"))%>%
summarise(
    count_below_66_range = sum(below_66_range, na.rm = TRUE),
    count_in_66_85_range = sum(in_66_85_range, na.rm = TRUE),
    count_above_85_range = sum(above_85_range, na.rm = TRUE),
  ...
  mutate(perc_66_85 = round((count_in_66_85_range / total_n) * 100, 1))%>%
  mutate(perc_above85e = round((count_above_85_range / total_n) * 100, 1))%>%
  mutate(TG_analysis = "TG475_UnGrouped")
  
rbind(EFSAgenotoxDB_inVivo_probs_sum_474,EFSAgenotoxDB_inVivo_probs_sum_475)
write_xlsx(rbind(EFSAgenotoxDB_inVivo_probs_sum_474,EFSAgenotoxDB_inVivo_probs_sum_475), here::here("tables", "EFSAgenotoxDB_inVivo_probs_sum.xlsx"))
```
...
summarise(
  ...
  mutate(TG_analysis ="TG474_GroupedByRef")
